...
uvicorn app.main:app --reload --host 0.0.0.0 --port 8001 # explicit local server
```
...
docker run --rm -p 8001:8001 --env-file .env receipt-ocr
```
...
host. The repository's AWS workflow publishes it to Amazon ECR:

```bash
docker pull <account>.dkr.ecr.<region>.amazonaws.com/nibblai-ai:<tag>
docker run -d --restart unless-stopped -p 8001:8001 \
  -e API_KEY=<secret> <account>.dkr.ecr.<region>.amazonaws.com/nibblai-ai:<tag>
```
...
|---|---|---|
| `OCR_PROVIDER` | `tesseract` | The Docker image includes Tesseract and Paddle. `paddleocr` is PP-OCRv5 (seconds on CPU). `paddleocr_vl` is the 0.9B VLM and needs a GPU. `fixture` replays recorded OCR and refuses to run in production. |
...
| `HOST` / `PORT` | `127.0.0.1` / `8001` | Bind address for the development server only. |
