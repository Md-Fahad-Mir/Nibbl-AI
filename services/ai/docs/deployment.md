...
docker run --rm -p 8001:8001 --env-file .env receipt-ocr:1.0.0
```

The image is multi-stage: the builder creates a locked virtualenv from
`pyproject.toml` and `uv.lock`, and the runtime copies only that virtualenv plus
